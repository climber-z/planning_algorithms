        ...
        for(int j=0;j <= _discretize_step; j++){        //acc_input_ay
            ...
            for(int k=0; k <= _discretize_step; k++){   //acc_input_az
                ...
                for(int step=0 ; step<=_time_step ; step ++){
                    ...
                    use the pos and vel to record the steps in the trajectory
                    */

                    pos(0) = pos(0) + delta_time * vel(0) + 0.5 * acc_input(0) * delta_time * delta_time;
                    pos(1) = pos(1) + delta_time * vel(1) + 0.5 * acc_input(1) * delta_time * delta_time;
                    pos(2) = pos(2) + delta_time * vel(2) + 0.5 * acc_input(2) * delta_time * delta_time;
                    ...
                    //check if the trajectory face the obstacle
